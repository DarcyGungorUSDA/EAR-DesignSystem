...
And unfortunately, CEC can't provide an FNA-wide solution (...I tried in 2025 when developing the ~FNS~ FNA PowerPoint template). Each of us *could* logon to Adobe.com with our email address, add the **Source Sans Pro** font family to our account, and edit the pdfs online - but it's a lot to assume everyone will do that.
...
**Public Sans, Roboto Mono, Merriweather**: These are the fonts the [U.S. Web Design System](https://designsystem.digital.gov/components/typography/#included-typefaces-2) recommends for government websites, in addition to **Source Sans Pro**. The only one we have access to in MS Office is **Merriweather**, but it's a serif typeface like **Times New Roman**. If people love it, we can use it for headings - but then it would look a little different from our website, which uses **Source Sans Pro** for headings. (As of 2025, the USDA webiste uses **Merriweather**, though.)  
**Verdana**: This typeface is an *excellent* alternative to **Arial**, and often promoted as a good choice for readers with dyslexia. I didn't pick it because the FNA PowerPoint template uses Arial (Verdana takes up more space, which is limited on a slide), and it's helpful to have a consistent typeface across all of our files. 
